...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, hidden_states):
        ...
        # Should run lstm over the inputs
        # Should pass the lstm outputs to the mlp
        # Should compute the log probabilities from the outputs of the mlp
        # Should also output the hidden states after the final sequence in the batch has been processesd.
...
#         print(f"embedded_inputs permuted shape:{embedded_inputs.shape}")
        
        
        # Passing the inputs to the lstm
        lstm_output, states = self.lstm(embedded_inputs, hidden_states)
#         print(f"lstm output shape:{lstm_output.shape}")
#         print(f"lstm hidden states shape:{states[0].shape}")
        
        
        # Passing lstm outputs to MLP
...
#         print(f"log-probas reshaped shape:{log_probas_reshaped.shape}")
        
        # Computing the nll_loss WITHOUT reduction
        all_loss = F.nll_loss(log_probas_reshaped, targets.reshape(-1), reduction='none')
#         print(f"losses shape:{all_loss.shape}")
        
        all_loss_reshaped = all_loss.view(shapes[0],shapes[1])
#         print(f"losses reshaped shape:{all_loss_reshaped.shape}")
        
        losses_masked = all_loss_reshaped*mask/mask.sum(1, keepdim=True)
#         losses_masked = all_loss_reshaped/mask.sum(1, keepdim=True)
        
        return losses_masked.sum()/shapes[0]
