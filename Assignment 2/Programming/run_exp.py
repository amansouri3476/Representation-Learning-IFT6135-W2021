...
def train(epoch, model, dataloader, optimizer, args):
    ...
    for idx, batch in enumerate(
        ...
        if idx % 5*args.print_every == 0:
            log_dir = os.path.join(args.log_dir, args.exp_id)
            filename = os.path.join(log_dir, "gpu_memory.txt")
            submit_command = (f"nvidia-smi --query-compute-apps=used_memory --format=csv >> {filename}")
            exit_status = subprocess.call(submit_command, shell=True)
            with open(filename,'a') as f:
                f.write(f"\n"+str(torch.cuda.get_device_properties(device=None))+f"\n\n")
...
def main(args):
    ...
    # Scheduler
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=0, threshold=0.1)
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Before scheduler loss is: {loss}")
        scheduler.step(loss)
